...
def parse_args():
    ...
    parser.add_argument('--zoom', default=4, type=int)
...
def main():
    ...
    # load the image
    img_true_np = bu.read_true_image(SR, IMG_STEM)
    img_noisy_np, _ = bu.read_noisy_image(IMG_STEM, zoom=ZOOM)
    ...
    downsampler = su.get_downsampler(zoom=ZOOM, in_channels=in_channels).type(DTYPE)
    ...
    datadir = ROOT[BENCHMARK][SR][ZOOM][IMG_STEM]
    ...
    for i, model_name in enumerate(model_names, start=1):
        ...
        if CHECK and \
            ...
            modeldir['input_noise.npy'].exists():
